...
if (isset($_GET['restore_post'])) {
  $id = (int)$_GET['restore_post'];
  $conn->query("UPDATE posts SET deleted_at = NULL WHERE id = $id");
...
<head>
  ...
  <style>
    /* Glitch Cursor Background Effect */
...
<body class="bg-gray-900 text-white min-h-screen p-6 relative overflow-hidden">
  ...
  <div class="grid md:grid-cols-2 gap-8 relative z-10">
    ...
    <div class="bg-gray-800 p-6 rounded-xl">
      <h2 class="text-xl font-semibold mb-4">Deleted Posts</h2>
      <ul class="space-y-4">
        <?php
        $posts = $conn->query("SELECT * FROM posts WHERE deleted_at IS NOT NULL ORDER BY deleted_at DESC");
        while ($post = $posts->fetch_assoc()):
        ?>
          <li class="bg-gray-700 p-4 rounded-lg">
            <h3 class="font-bold text-lg"><?= htmlspecialchars($post['title']) ?></h3>
            <p class="text-sm text-gray-400 mb-2">Deleted on <?= date("M j, Y", strtotime($post['deleted_at'])) ?></p>
            <div class="flex gap-2">
              <a href="?restore_post=<?= $post['id'] ?>" class="bg-green-600 hover:bg-green-700 px-3 py-1 rounded text-sm">Restore</a>
              <button onclick="showPasscodeModal('<?= $post['id'] ?>', 'post')" class="bg-red-600 hover:bg-red-700 px-3 py-1 rounded text-sm">Delete Forever</button>
            </div>
          </li>
        <?php endwhile; ?>
      </ul>
